...
% Jean-Yves Tinevez <tinevez AT pasteur DOT fr> March 2012
    ...
    parser.addParamValue('LineWidth', 2, @(x) isnumeric(x) && isscalar(x) );
    parser.addParamValue('Sorted', false, @(x) islogical(x) && numel(x) ==1 );
    parser.addParamValue('DrawLabels', true, @(x) islogical(x) && numel(x) ==1 );
    parser.addParamValue('XOffset', 0, @(x) isnumeric(x) && isscalar(x));
    parser.addParamValue('YOffset', 0, @(x) isnumeric(x) && isscalar(x));
...
%             'XLim', [0 maxXpos * 1.05]);
    ...
    for i = iterator
        ...
        hl = line([x1 x2]+xoffset, [y1 y2]+yoffset, ...
            'Color', LINE_COLOR, ...
            'LineWidth', linewidth);
    ...
    for i = iterator
        ...
        if drawlabels
            ...
            ht = text(x1+xoffset, y2+yoffset, contentfun(content), ...  A hack to have text displayed above bars
        ...
        if numel(children) > 1
            ...
            hl = line(x1+xoffset, y1+yoffset, ...
                'Color', LINE_COLOR, ...
                'Marker', '.', ...
                'MarkerSize', 14, 'Linewidth', linewidth);
